...
from src.test.databus import DataBus
from src.proc_modes.fft3d_window_1 import FFT3DWindow
import sys
import os

PROJECT_ROOT = r"C:\Users\EMI_user1\Desktop\radar\TinyRadTool"

if PROJECT_ROOT not in sys.path:
    sys.path.append(PROJECT_ROOT)

print(">>> USING NEW ModRangeDoppler_1 <<<")
with open(r"C:\Users\EMI_user1\Desktop\radar\python_start_test.txt", "w") as f:
    f.write("Python reached ModRangeDoppler_1.py")

# Use ONLY the new 3D window (Option A)

# Shared data bus for real-time frames

# TinyRad + processing modules

# PyQt5 + UI

MATLAB_FILE = r"C:\Users\EMI_user1\Desktop\radar\TINYRADTOOL\live_radar_data.mat"
...
class ModRangeDoppler(QtWidgets.QMainWindow, ui_ModRangeDoppler.Ui_ModRangeDoppler):
    ...
    def __init__(self, patre):
        ...
        # Keyboard shortcuts
        QtWidgets.QShortcut(
            QtGui.QKeySequence("F10"), self.Widget_Meas, self.toggleHistog
        )
        QtWidgets.QShortcut(
            QtGui.QKeySequence("ALT+F2"), self.Widget_Meas, self.ToggleCmd
        )
        QtWidgets.QShortcut(
            QtGui.QKeySequence("Up"), self.Widget_Meas, self.SetCmdTxtUp
        )
        QtWidgets.QShortcut(
            QtGui.QKeySequence("Down"), self.Widget_Meas, self.SetCmdTxtDown
        )
        ...
        # Default signal processing config
        ...
        # Load config file if GUI started with one
    ...
    def LimFreq(self):
        ...
        if fMin < self.patre.Lim_fMin:
            ...
            self.UpdIniAppend(
                "Func", "self.Edit_Cfg_RadStrtFreq.setText", str(fMin / 1e6)
            )
        if fMax > self.patre.Lim_fMax:
            fMax = self.patre.Lim_fMax
            self.UpdIniAppend(
                "Func", "self.Edit_Cfg_RadStopFreq.setText", str(fMax / 1e6)
            )
            ...
            with h5py.File(stFile, "w") as hFile:
                ...
                for Key in self.dBrdCfg:
                    ...
                    if isinstance(self.dBrdCfg[Key], str):
                        ...
                        DatSet = hFile.create_dataset(
                            "/BrdCfg/" + str(Key), (100, 1), dtype=dt
                        )
                        DatSet.attrs["name"] = self.dBrdCfg[Key]
                    else:
                        hFile.create_dataset(
                            "/BrdCfg/" + str(Key), data=self.dBrdCfg[Key]
                        )
    ...
    def SigActionTimerStart(self):
        ...
        print("TIMER START FUNCTION CALLED")
    ...
    def Button_Meas_Ini_Clicked(self):
        ...
        if not RetVal[0]:
            ...
            Ret = 0
            self.Log.Append(
                "Application running" + str(RetVal[0]) + " | " + str(RetVal[1])
            )
            palette = QtGui.QPalette()
            palette.setColor(QtGui.QPalette.Foreground, QtCore.Qt.darkGreen)
            self.UpdIniAppend("Func", "self.Label_Meas_CfgInf.setPalette", palette)
            self.UpdIniAppend(
                "Func", "self.Label_Meas_CfgInf.setText", "Application running"
            )
        ...
        if Ret == 0:
            ...
            self.UpdIniAppend("Func", "self.Label_Meas_CfgInf.setPalette", palette)
            self.UpdIniAppend(
                "Func", "self.Label_Meas_CfgInf.setText", "Connected: Initializing..."
            )
        ...
        if IniSts == 1:
            ...
            self.stCfgInf = (
                "Range-Doppler Cfg: fs = "
                + str(fs)
                + " (MHz) | N = "
                + str(self.dBrdCfg["N"])
                + " | Tp="
                + str(np.round(10 * self.dBrdCfg["Tp"] / 1e-6) / 10)
                + " us"
            )
        ...
        with open(r"C:\Users\EMI_user1\Desktop\radar\radar_matrix.txt", "w") as f:
            f.write("Radar file created successfully\n")

        print("Initialization Done")

    def SigActionInitDone(self):
        print("SigActionInitDon executed: ", self.MeasIni)
        if self.MeasIni is not None:
            self.Button_Meas_Meas.setDisabled(False)
        else:
            if self.InitFailed:
                self.Button_Meas_Meas.setDisabled(True)
    ...
    def MeasUpdate(self):
        ...
        print("MEASUPDATE CALLED")

        with open(r"C:\Users\EMI_user1\Desktop\radar\test_measupdate.txt", "a") as f:
            f.write("MEASUPDATE CALLED\n")
        ...
        if NrFrms < 0:
            ...
            value = int(float(100 * (self.ProcFrms) / self.dBrdCfg["NrFrms"]))
    ...
    def Show3DRangeDoppler(self):
        ...
        try:
            RD = np.copy(self.RDMap)

            print("\n===================================")
            print("3D RANGE DOPPLER INFORMATION")
            print("===================================")
            print("RD Matrix Shape :", RD.shape)
            print("RD Matrix Size  :", RD.nbytes, "bytes")

        except Exception as e:
            print("3D Plot Error:", e)

    def GetCicCfg(self):

        dIniCic = {"FiltSel": 255, "CombDel": 0, "OutSel": 0, "SampPhs": 0}
    ...
    def GetRangeDopplerCfg(self):
        ...
        if dIniRangeDoppler["Np"] < 8:
            ...
            self.UpdIniAppend(
                "Func", "self.Edit_Cfg_RadNp.setText", str(dIniRangeDoppler["Np"])
            )
        ...
        if 4 * dIniRangeDoppler["Np"] * dIniRangeDoppler["N"] > int("0x60000", 16) / 2:
            dIniRangeDoppler["Np"] = int(
                np.floor(int("0x60000", 16) / (2 * 4 * dIniRangeDoppler["N"]))
            )
            self.UpdIniAppend(
                "Func", "self.Edit_Cfg_RadNp.setText", str(dIniRangeDoppler["Np"])
            )
        ...
        if (
            dIniRangeDoppler["TInt"]
            < dIniRangeDoppler["TimPerd"] * dIniRangeDoppler["Np"]
        ):
            dIniRangeDoppler["TInt"] = dIniRangeDoppler["TimPerd"] * (
                dIniRangeDoppler["Np"] + 2
            )
    ...
    def IniRDMap(self):
        ...
        FreqDisp = np.linspace(
            0, int(self.SigCfgRFFT / 2 - 1), int(self.SigCfgRFFT / 2)
        )
        ...
        self.SigCfgRScale = (self.SigCfgRMax - self.SigCfgRMin) / (
            self.SigCfgRIdxMax - self.SigCfgRIdxMin
        )

        freqDoppler = (
            np.linspace(-self.SigCfgVFFT / 2, self.SigCfgVFFT / 2 - 1, self.SigCfgVFFT)
            / self.SigCfgVFFT
            / self.dBrdCfg["TimPerd"]
        )
        ...
        self.SigCfgVelScale = (
            self.VelDoppler[self.SigCfgVFFT - 1] - self.VelDoppler[0]
        ) / self.SigCfgVFFT
    ...
    def GetBrdCfg(self, dIniAdc, dIniRangeDoppler):
        ...
        self.UpdIniAppend(
            "Func",
            "self.Edit_Cfg_RadDur.setText",
            str(np.floor(dBrdCfg["TimUp"] / 1e-6 * 10) / 10),
        )
        self.UpdIniAppend(
            "Func",
            "self.Edit_Cfg_RadPerd.setText",
            str(np.floor(dBrdCfg["Tp"] / 1e-6 * 10) / 10),
        )
...
def CalcRangeDopplerMap(self, Data):
    ...
    np.savetxt(
                r"C:\Users\EMI_user1\Desktop\radar\RawData_Modulated.csv",
                Data,
                delimiter=","
            )
    ...
    if self.SigCfg_RangeWinType == "Hanning":
        Win = np.hanning(N - 2)
    elif self.SigCfg_RangeWinType == "Hamming":
        Win = np.hamming(N - 2)
    else:
        Win = np.ones(N - 2)
    ...
    np.savetxt(
                r"C:\Users\EMI_user1\Desktop\radar\RangeFFT.csv",
                np.abs(XData),
                delimiter=","
            )

    print("Range FFT Size =", XData.shape)
    #################################################
    # SAVE RANGE FFT MATRIX
    #################################################

    print("Range FFT Size =", XData.shape)

    np.savetxt(
    r"C:\Users\EMI_user1\Desktop\radar\RangeFFT.csv", np.abs(XData), delimiter=","
    )

    print("RangeFFT.csv Saved")
    ...
    elif self.SigCfg_VelWinType == "Hamming":
    ...
    #################################################
    # Doppler FFT (2D FFT second stage)
    #################################################
    ...
    XDataFFT = np.fft.fftshift(XData, 1)

    #################################################
    # SAVE COMPLEX 2D FFT MATRIX
    #################################################

    print("2D FFT Matrix Size =", XDataFFT.shape)

    print("First 5x5 Complex Elements")
    print(XDataFFT[0:5, 0:5])

    np.savetxt(
    r"C:\Users\EMI_user1\Desktop\radar\RD_PYTHON_COMPLEX_REAL.csv",
    np.real(XDataFFT),
    delimiter=",",
    )

    np.savetxt(
    r"C:\Users\EMI_user1\Desktop\radar\RD_PYTHON_COMPLEX_IMAG.csv",
    np.imag(XDataFFT),
    delimiter=",",
    )
...
def ParseCfgFile(self, stFile):
    if len(stFile) > 1:
        ...
        lCmd.append(
            (
                r"PageRangeDopplerCfg_SetStrtFreq\((?P<Arg>([0-9]+[.]{0,1}[0-9]*))\)",
                "self.Edit_Cfg_RadStrtFreq.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerCfg_SetStopFreq\((?P<Arg>([0-9]+[.]{0,1}[0-9]*))\)",
                "self.Edit_Cfg_RadStopFreq.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerCfg_SetSamples\((?P<Arg>([0-9]+[.]{0,1}[0-9]*))\)",
                "self.Edit_Cfg_RadSamples.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerCfg_SetChirpRep\((?P<Arg>([0-9]+[.]{0,1}[0-9]*))\)",
                "self.Edit_Cfg_RadPerd.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerCfg_SetNrChirp\((?P<Arg>([0-9]+))\)",
                "self.Edit_Cfg_RadNp.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerCfg_SetTInt\((?P<Arg>([0-9]+))\)",
                "self.Edit_Cfg_RadTInt.setText",
                "Func",
            )
        )
        ...
        lCmd.append(
            (
                r"PageRangeDopplerCfg_EnaNormMax\((?P<Arg>(True|False))\)",
                "self.ChkBox_SigCfg_NormMax.setChecked",
                "FuncSt",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerCfg_SetNormLimMin\((?P<Arg>(-[0-9]+[.]{0,1}[0-9]*))\)",
                "self.Edit_Cfg_LimMaxMin.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerCfg_SetNormSidelobe\((?P<Arg>([0-9]+[.]{0,1}[0-9]*))\)",
                "self.Edit_Cfg_LimMaxSideLev.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerCfg_EnaLimMinMax\((?P<Arg>(True|False))\)",
                "self.ChkBox_SigCfg_NormFixed.setChecked",
                "FuncSt",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerCfg_SetLimMin\((?P<Arg>(-[0-9]+[.]{0,1}[0-9]*))\)",
                "self.Edit_Cfg_LimFixedMin.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerCfg_SetLimMax\((?P<Arg>(-[0-9]+[.]{0,1}[0-9]*))\)",
                "self.Edit_Cfg_LimFixedMax.setText",
                "Func",
            )
        )

        # Set Parameters for Number of Parameters for RangeDoppler
        # measurement page
        lCmd.append(
            (
                r"PageRangeDopplerMeas_SetNrFrms\((?P<Arg>([0-9]+))\)",
                "self.Edit_Meas_NrFrms.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerMeas_SetRMin\((?P<Arg>([0-9]+[.]{0,1}[0-9]*))\)",
                "self.Edit_Meas_RMin.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerMeas_SetRMax\((?P<Arg>([0-9]+[.]{0,1}[0-9]*))\)",
                "self.Edit_Meas_RMax.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerMeas_EnaFFT\((?P<Arg>(True|False))\)",
                "self.ChkBox_Meas_FFT.setChecked",
                "FuncSt",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerMeas_EnaAverage\((?P<Arg>(True|False))\)",
                "self.ChkBox_Meas_PSD.setChecked",
                "FuncSt",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerMeas_SetTxCfg\('(?P<Arg>([0-9a-zA-Z \(\);]+))'\)",
                "self.Edit_Meas_Cfg.setText",
                "Func",
            )
        )
        ...
        lCmd.append(
            (
                r"PageRangeDopplerMeas_SetRangeFFTSiz\((?P<Val>(64|128|256|512|1024|2048|4096|8192|16384|32768|65536))\)",
                "self.Edit_Cfg_RangeFftSiz.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerMeas_SetRangeWinType\((?P<Val>(Hanning|Hamming|Boxcar))\)",
                "self.Edit_Cfg_RangeWinType.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerMeas_SetVelFFTSiz\((?P<Val>(64|128|256|512|1024|2048|4096|8192|16384|32768|65536))\)",
                "self.Edit_Cfg_VelFftSiz.setText",
                "Func",
            )
        )
        lCmd.append(
            (
                r"PageRangeDopplerMeas_SetVelWinType\((?P<Val>(Hanning|Hamming|Boxcar))\)",
                "self.Edit_Cfg_VelWinType.setText",
                "Func",
            )
        )
